...
const displacement = {}
displacement.boundingBox = new THREE.Mesh(new THREE.BoxGeometry(4,5,14),new THREE.MeshBasicMaterial({wireframe:true}))
displacement.boundingBox.position.set(1,1,1)
scene.add(displacement.boundingBox)
...
gpgpu.particlesVariable.material.uniforms.uDisplacement = new THREE.Uniform(new THREE.Vector3(9999,9999,9999))
...
const tick = () =>
{
    ...
    const intersetion = displacement.raycaster.intersectObject(displacement.boundingBox)
    if(intersetion.length){
        const location = intersetion[0].point
        gpgpu.particlesVariable.material.uniforms.uDisplacement.value.copy(location)
    }
